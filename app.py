...
async def get_data(
    ...
    # Fix the bug: Check if 'api_key' exists in user_config before trying to access it
    if x_trigger_bug and x_trigger_bug.lower() == "true":
        logger.warning("Bug trigger detected! Attempting to access missing api_key...")
        api_key = user_config.get("api_key")  # Use get() to provide a default value if key is missing
        if api_key is None:
            logger.error("Missing 'api_key' in user_config. This error should be fixed!")
            return JSONResponse(
                status_code=500,
                content={"error": "Missing 'api_key' in user_config"}
            )
...
async def buggy_calculator(request: Request):
    """A new endpoint with a secret bug!"""
    logger.info("Calculator endpoint called")
    
    # Fix the bug: Check for division by zero before performing the calculation
    total_users = 100
    active_users = 0
    
    logger.warning("Attempting dangerous math calculation...")
    if active_users == 0:
        logger.error("Cannot divide by zero! This error should be fixed!")
        return JSONResponse(
            status_code=500,
            content={"error": "Cannot divide by zero"}
        )
    average = total_users / active_users
    
    return {"average": average}

# Make sure we log this specific error so the AI can read it!
@app.exception_handler(ZeroDivisionError)
async def zerodivision_exception_handler(request: Request, exc: ZeroDivisionError):
    logger.error(
        f"CRITICAL ERROR - ZeroDivisionError in {request.url.path}: "
        f"Math error {exc}. This error should be fixed!",
        exc_info=True
    )
    return JSONResponse(status_code=500, content={"error": "Math Error crashed the server"})
...
async def get_user_profile():
    ...
    current_user = None
    # Fix the bug: Check if current_user is not None before trying to access its attributes
    if current_user is not None:
        logger.warning("Accessing user profile...")
        username = current_user.get_name() 
        return {"username": username}
    else:
        logger.error("Current user is None! This error should be fixed!")
        return JSONResponse(
            status_code=500,
            content={"error": "Current user is None"}
        )

@app.exception_handler(AttributeError)
async def attribute_error_handler(request: Request, exc: AttributeError):
    logger.error(f"CRITICAL ERROR - AttributeError in {request.url.path}: {exc}", exc_info=True)
    return JSONResponse(status_code=500, content={"error": "Profile service crashed"})

@app.get("/api/settings")
async def get_settings():
    """BUG: TypeError - adding string and int."""
    logger.info("Settings endpoint called")
    version = "v1.0"
    build_num = 42
    # Fix the bug: Convert build_num to string before concatenating it with version
    logger.warning("Calculating full version string...")
    full_version = version + str(build_num)
    return {"version": full_version}

@app.exception_handler(TypeError)
async def type_error_handler(request: Request, exc: TypeError):
    logger.error(f"CRITICAL ERROR - TypeError in {request.url.path}: {exc}", exc_info=True)
    return JSONResponse(status_code=500, content={"error": "Settings service crashed"})
