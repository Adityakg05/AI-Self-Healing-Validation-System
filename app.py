...
async def get_data(
    ...
    # THE FIX: Check if api_key exists before attempting to access it
    if x_trigger_bug and x_trigger_bug.lower() == "true":
        logger.warning("Bug trigger detected! Attempting to access missing api_key...")
        # Check if api_key exists in user_config before accessing it
        if "api_key" in user_config:
            api_key = user_config["api_key"]
            logger.info(f"Using API key: {api_key}")
        else:
            logger.error("Missing API key in user configuration!")
            return DataResponse(
                data=user_config,
                message="Missing API key in user configuration",
                timestamp=datetime.now(timezone.utc).isoformat()
            )
...
async def buggy_calculator(request: Request):
    ...
    # THE BUG: Math error (dividing by zero)
    total_users = 100
    active_users = 0
    
    logger.warning("Attempting dangerous math calculation...")
    # This will crash with a ZeroDivisionError!
    if active_users != 0:
        average = total_users / active_users
    else:
        logger.error("Cannot calculate average with zero active users!")
        return {"error": "Cannot calculate average with zero active users"}
