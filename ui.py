#!/usr/bin/env python3
"""
Streamlit UI for AI-Self-Healing-Validation-System.

Interactive interface to:
1. Start/stop FastAPI app
2. Trigger crashes
3. Monitor workflow
...
# Page config
st.set_page_config(
    page_title="AI-Self-Healing-Validation-System",
    page_icon="🤖",
    layout="wide",
...
def trigger_crash() -> tuple[bool, str]:
    ...
    try:
        backend_url = os.getenv("BACKEND_URL", f"http://127.0.0.1:8000")
        # Send request with bug trigger header to intentionally cause error
        response = httpx.get(
...
def run_workflow():
    """Execute self-healing workflow with updates."""
...
def main():
    ...
    # Header
    st.markdown('<h1 class="main-header">🤖 AI-Self-Healing-Validation-System</h1>', unsafe_allow_html=True)
    st.markdown(
