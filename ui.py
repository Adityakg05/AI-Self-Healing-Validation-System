...
def check_app_running() -> bool:
    ...
    try:
        response = httpx.get(f"http://127.0.0.1:{settings.app_port}/health", timeout=2.0)
...
def trigger_crash() -> tuple[bool, str]:
    ...
    try:
        response = httpx.get(
            f"http://127.0.0.1:{settings.app_port}/api/data",
...
"""
    ...
    try:
        ...
        # Get final state from our accumulated tracker
